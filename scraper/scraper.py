...
def read_question(num):
    session = requests.Session()
    session.get(f"https://regeltest.swissunihockey.ch/de/online-test/training?question_no={num}")
    response = session.get('https://regeltest.swissunihockey.ch/de/online-test/training?check=Weiter+%C2%BB')
